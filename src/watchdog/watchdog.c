...
LOG_MODULE_REGISTER(watchdog, CONFIG_WATER_QA_LOG_LEVEL);

#define WDT_FEED_WORKER_DELAY_MS \
	((CONFIG_WATER_QA_WATCHDOG_TIMEOUT_MSEC)/2)
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	static const struct wdt_timeout_cfg wdt_settings = {
			.window = {
				.min = 0,
				.max = CONFIG_WATER_QA_WATCHDOG_TIMEOUT_MSEC,
			},
			.callback = NULL,
	...
	LOG_INF("Watchdog timeout installed. Timeout: %d",
		CONFIG_WATER_QA_WATCHDOG_TIMEOUT_MSEC);
	return 0;
}
